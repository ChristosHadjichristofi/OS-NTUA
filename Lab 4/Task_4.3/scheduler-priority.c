...
//change the priority of a process from low to high and relocate it in the queue
static int sched_high_task_by_id(int id){
	...
	temp->priority='h';
	//if it was the only process with low priority then it stays at the tail of the queue
	if((temp==tail)&&(prev_temp->priority='h'))
		return 0;
	prev_temp->next=temp->next;	
	//if it was at the tail of the queue but there are more processes with low priority, tail changes
	if(temp==tail)
		tail=prev_temp;
	//finds the last process with high priority and places the current process there
...
//change the priority of a process from high to low and relocate it in the queue
static int sched_low_task_by_id(int id){
	...
	temp->priority='l';
	//if it is already at the tail of the queue then it doesn't change place
	if(temp==tail)
		return 0;
...
static void sigchld_handler(int signum){
	...
	for(;;){
		...
		if (WIFEXITED(status) || WIFSIGNALED(status) || WIFSTOPPED(status)){
			...
			//if the current process is the shell then start the next process no matter what	
			if(it==head){
				it = it->next;
				kill(it->pid,SIGCONT);
				alarm(SCHED_TQ_SEC);
			}
			//if the current process has high priority then start the next process only if it also has high priority
			...
			//if the current process has low priority then start the next process no matter what
			else{
				it = it->next;
				kill(it->pid,SIGCONT);
				alarm(SCHED_TQ_SEC);
